...
export function addColorToHistory(hex: string): void {
	...
	if (windowState.colorHistory.length > MAX_HISTORY_ITEMS) {
		console.log(
			`[Window Manager] History exceeded ${MAX_HISTORY_ITEMS} items, trimming to limit`,
		);
		windowState.colorHistory = windowState.colorHistory.slice(
			0,
			MAX_HISTORY_ITEMS,
		);
	}
...
export function setColorHistory(history: ColorHistoryItem[]): void {
	windowState.colorHistory = history.slice(0, MAX_HISTORY_ITEMS);
	if (exploreWindow && !exploreWindow.isDestroyed()) {
		exploreWindow.webContents.send("history-updated", windowState.colorHistory);
	}
}
...
/**
 * Resize the explore window (used when the color wheel drawer opens/closes)
 */
export function setExploreWindowSize(width: number, height: number): void {
	if (exploreWindow && !exploreWindow.isDestroyed()) {
		exploreWindow.setSize(width, height, true);
	}
}
